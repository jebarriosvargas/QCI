...
Una partícula libre es aquella que no está sometida a una fuerza, por lo que la energía potencial de una partícula es constante para todo valor de $x$ ($F_x=dU/dx=0 \Rightarrow U\iprn x \fprn = {\rm cte.}$). Dado que tenemos la libertad de elección del nivel cero de energía, podemos elegir que $U\iprn x \fprn = {\rm cte.} = 0$, por lo que la ecuación de Schrödinger independiente (@eq-schroedinger-independiente-tiempo) esta dada por,
...
for n in range(1,npts_t):
    ...
    cuadros.append( go.Frame(data=[go.Scatter(x=X, y=Ψ.real)],layout=go.Layout(title=f"t={T[n]:.2f} (fs)")) )
...
# d = {'x':x,'t':t,'Repsi':Ψ.real,'Impsi':Ψ.imag}
# df = pd.DataFrame(data=d)

# px.scatter(df, x="x", y="Repsi", animation_frame="t")
